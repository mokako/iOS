...
-(UITextView *)text
{
    UITextView *ob = [[UITextView alloc]initWithFrame:CGRectMake(0,-30,40,20)];
...
-(UITextView *)widthText
{
    UITextView *ob = [[UITextView alloc]initWithFrame:CGRectMake(50,-9,40,20)];
    ob.textColor = RGB(255, 255, 255);
    ob.backgroundColor = RGBA(255, 255, 255, 0.0);
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if((currentPoint.y - 40) >= 0 && (currentPoint.y - 40) <= 127.5)
    {
        ...
        value.text = [NSString stringWithFormat:@"%f",currentPoint.x - 100];
    }
    ...
    if(TOUCH_COLOR != 0 && TOUCH_COLOR != 5){
        ...
        if(currentPoint.x >= 100 && currentPoint.x <= 240){
            if(currentPoint.x - 100 == 0){
                ...
                value.text = [NSString stringWithFormat:@"%f",0.5];
            }else{
                touchView.center = CGPointMake((currentPoint.x - 100),20);
